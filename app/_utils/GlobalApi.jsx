...
const axiosClient = axios.create({
  ...
  // baseURL: 'http://192.168.211.57:1337/api'
})

// const axiosClient1 = axios.create({
//   // baseURL: 'http://localhost:1337/api'
//   baseURL: 'http://192.168.211.57:1337'
// })
...
const getCartItems = (userid, jwt) =>axiosClient.get('/user-carts?filters[userid][$eq]='+userid+'&[populate][products][populate][images][populate][0]=url', {
  ...
  const cartItemLists = data.map((item, index)=>({
...
const deleteCartItem = (id, jwt) => {
  return axiosClient.delete(`/user-carts/${id}`, {
    headers: {
      Authorization: `Bearer ${jwt}` // Ensure correct JWT format
    }
  });
};
...
const getMyOrder = (userId, jwt) => axiosClient.get('/orders?filters[userid][$eq]='+userId+'&populate[orderItemList][populate][products][populate][images]=url',{
  headers:{
    Authorization:'Bearer '+jwt
  }
}).then(resp=>{
  ...
  const orderList = responce.map((item, index) =>({
    id:item.id,
